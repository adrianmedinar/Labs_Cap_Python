"""
Demo end-to-end:
  1. Crea el esquema en SQLite en memoria (equivalente a lo que
     Alembic haria via 'alembic upgrade head' en una BD persistente).
...
def main() -> None:
    ...
    try:
        # ---------------- CREATE ----------------
        print_header("CREATE")
        user = crud.create_user(session, username="amedina", email="amedina@axity.com")
        ...
        # ---------------- READ ----------------
        print_header("READ")
        found_user = crud.get_user_by_username(session, "amedina")
        ...
        crud.update_user_email(session, user.id, "amedina@axity.com")
